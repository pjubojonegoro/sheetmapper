...
MeasureControl.prototype._bindPointPopup = function(list) {
    ...
    list.map(function(point) {
        ...
        var popup = new mapboxgl.Popup({
            ...
            .setHTML(point.properties.curDistance + "km")
            .addTo(this.map);
...
MeasureControl.prototype._onDblClick = function(e){
    ...
    this.geojsonList[this._name].popups[length - 1].setHTML("<div>Total：" + options.total + "km<button class='ruler' data-name='"+this._name+"' id='dele-btn-"+this._name+"'>X</button></div>");
...
MeasureControl.prototype._onAddMouseTips = function(e){
    var tips = this.tips =  this._createNode('div','measure_mousemove_tips','Ukur Jarak',document.body,'mousemove_tips');
    //设置tips的样式
...
MeasureControl.prototype._onMousemove = function(e){
    ...
    this.tips.innerText = 'Jarak：'+ distance + 'km';
}
